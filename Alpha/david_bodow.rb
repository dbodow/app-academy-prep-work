...
def missing_numbers(nums)
  # your code goes here
  smallest, largest = nums.minmax
  out = []
  smallest.upto(largest) do |x|
    out << x unless nums.include?(x)
  end
  out
end
...
def base2to10(binary)
  # your code goes here
  binary_arr = binary.chars.map { |x| x.to_i }.reverse
  out = 0
  binary_arr.each_with_index do |x, i|
    out += x * (2**i)
  end
  # debugger
  out
...
class Hash
  def just_simple_merge
    proc { |_k, _v_before, v_after| v_after }
  end

  def my_merge(hash, &prc)
    # your code goes here
    out = {}
    each { |k, v| out[k] = v } # copy by value
    hash.each do |k, v|
      begin
        if has_key?(k)
          out[k] = yield(k, self[k], v)
        else
          out[k] = just_simple_merge.call(k, self[k], v)
        end
      rescue
        p 'called default'
        out[k] = just_simple_merge.call(k, self[k], v)
...
p h1.my_merge(h3) { |key, oldval, newval| newval - oldval }
p h1.my_merge(h3) { |key, oldval, newval| newval - oldval } == { a: -5, b: 2, e: 6 }
p h1.my_merge(h3)
p h1.my_merge(h3) == { a: 5, b:2, e: 6 }
p h1.my_merge(h3) { |key, oldval, newval| oldval }
...
def even_palindrome(arr_of_cs, i_left)
  pal_length = 0
  pal = []
  i_right = i_left + 1
  furthest = min(i_left, arr_of_cs.length - 1 - i_right)
  0.upto(furthest).each do |shift|
    break if arr_of_cs[i_left - shift] != arr_of_cs[i_right + shift]
    pal.shift(arr_of_cs[i_left - shift])
    pal.push(arr_of_cs[i_right + shift])
    pal_length += 2
  end
  [pal_length, pal]
end

def odd_palindrome(arr_of_cs, i)
end

def longest_palindrome(string)
# your code goes here
arr_of_cs = string.chars

string.chars.each_with_index do |x, i|
  #find even palindrome length & odd palindrom length, then report the max of them
end
